...
def generate_visual_profile(blocks: List[Dict[str, Any]], extraction_metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Pass 1: Gather dimensions and areas
    for b in blocks:
        bbox = b.get("bbox")
    ...
    # Get actual dimensions or fallback to max extents
    ...
    # Pass 2: Headline detection requires final page_height
    for b in blocks:
        bbox = b.get("bbox")
        if not bbox or len(bbox) != 4:
            continue
        if b.get("block_type") == "text":
            x0, y0, x1, y1 = bbox
            height = max(0, y1 - y0)
            if y0 < (page_height * 0.2) and height > 20:
